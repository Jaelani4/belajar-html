...
<form action="{{ route('mahasiswa.update',$mhs->nim) }}" method="post">
    ...
    <div class="form-group">
        ...
        <input type="radio" name="jenis_kelamin" value="L" {{$mhs->jenis_kelamin == 'L' ? 'checked' : '' }}> Laki-laki<br>
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="jurusan_id">
            <option value="">--pilihan--</option>
            @foreach($jur as $j)
            <option disable="true" value="{{ $j->id }}" {{ ($j->id == $mhs->jurusan_id) ? 'selected' : '' }}>{{$j->jurusan}}
            </option>
            @endforeach
        </select>
    </div>
